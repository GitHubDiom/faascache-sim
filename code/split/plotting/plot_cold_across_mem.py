...
def plot_run(results_dict, num_funcs):
    ...
    for mem in results_dict.keys():
        ...
        for policy in results_dict[mem].keys():
            ...
            for func in analysis.keys():
                if func != "global":
                    accesses_per_policy[policy] += analysis[func]["accesses"]
                    misses_per_policy[policy] += analysis[func]["misses"]

            results_per_policy[policy].append((mem , misses_per_policy[policy] / accesses_per_policy[policy]))

    pth = os.path.join(plot_dir, "cold_drop_mem-{}.pdf".format(num_funcs))
